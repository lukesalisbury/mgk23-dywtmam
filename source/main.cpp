...
#include "resources/playsheet.hpp"
...
struct CodeOne {
	...
	char handle_filepath[512] = {0};
	char handle_filename[32] = {0};
...
ControllerPad default_gamepad = { 
	...
	{	CONTROLAXIS, 0, {-SDL_CONTROLLER_AXIS_LEFTX, SDL_CONTROLLER_AXIS_LEFTX, -SDL_CONTROLLER_AXIS_LEFTY, SDL_CONTROLLER_AXIS_LEFTY, SDL_CONTROLLER_AXIS_TRIGGERLEFT, SDL_CONTROLLER_AXIS_TRIGGERRIGHT}, {0,0,0}	},
	{	CONTROLAXIS, 0, {-SDL_CONTROLLER_AXIS_RIGHTX, SDL_CONTROLLER_AXIS_RIGHTX, -SDL_CONTROLLER_AXIS_RIGHTY, SDL_CONTROLLER_AXIS_RIGHTY, SDL_CONTROLLER_AXIS_TRIGGERLEFT, SDL_CONTROLLER_AXIS_TRIGGERRIGHT}, {0,0,0} },
	{	MOUSEAXIS, 0, 0, {0,0}, 0 }
		...
		*/
		...
		if ( print || sprite ) {
				...
				if ( cchar >= 32 && cchar <= 128 ) {
					...
					void * pointer = elix_hashmap_value_hash(&app->textures, sprite->id);
					selected_texture = (SDL_Texture*)pointer;
...
void DrawRenderItem( AppOne * app, RenderItem & item) {
	...
	if ( SDL_RenderCopyExF(app->renderer, texture, item.texture_area, &item.position, item.angle, nullptr, SDL_FLIP_NONE) == -1) {
		SDL_Log("DrawTextSimple: RenderCopy: %s", SDL_GetError());
	}
	...
	switch ( device ) {
		...
		case MOUSEBUTTON: {
			if ( symbol < 5 && symbol >= 0 ) {
				mouse_button = SDL_GetMouseState(&mouse_position[0], &mouse_position[1]);
				value = !!(mouse_button & SDL_BUTTON(symbol));
			}
			break;
		}
		case CONTROLAXIS: {
...
int8_t HandleEvents(AppOne * app, uint8_t previous_state) {
	...
	for (ControllerPad &pad : app->control_pads) {
		...
		switch (pad.right_stick.device) {
			case KEYBOARD: {
				int16_t key1, key2;
				key1 = UpdatGetInput(app, input_test, 13, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[0]);
				key2 = UpdatGetInput(app, input_test, 13, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[1]);
				pad.right_stick.value[0] = ( key1 && !key2 ? -255 : ( !key1 && key2 ? 255 : 0) );
	
				key1 = UpdatGetInput(app, input_test, 14, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[2]);
				key2 = UpdatGetInput(app, input_test, 14, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[3]);
				pad.right_stick.value[1] = ( key1 && !key2 ? -255 : ( !key1 && key2 ? 255 : 0) );

				key1 = UpdatGetInput(app, input_test, 15, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[4]);
				key2 = UpdatGetInput(app, input_test, 15, KEYBOARD, pad.left_stick.device_number,pad.left_stick.sym[5]);
				pad.right_stick.value[2] = ( key1 && !key2 ? -255 : ( !key1 && key2 ? 255 : 0) );
				break;
			}
			case CONTROLAXIS: {
				pad.right_stick.value[0] = UpdatGetInput(app, input_test, 13, CONTROLAXIS, pad.left_stick.device_number,pad.left_stick.sym[1]);
				pad.right_stick.value[1] = UpdatGetInput(app, input_test, 14, CONTROLAXIS, pad.left_stick.device_number,pad.left_stick.sym[3]);
				pad.right_stick.value[2] = UpdatGetInput(app, input_test, 15, CONTROLAXIS, pad.left_stick.device_number,pad.left_stick.sym[5]);
				break;
			}
			case MOUSEAXIS: {
				pad.right_stick.value[0] = UpdatGetInput(app, input_test, 13, MOUSEAXIS, pad.left_stick.device_number,pad.left_stick.sym[1]);
				pad.right_stick.value[1] = UpdatGetInput(app, input_test, 14, MOUSEAXIS, pad.left_stick.device_number,pad.left_stick.sym[3]);
				pad.right_stick.value[2] = 0;
				break;
			}
			default:
				break;
		...
		switch (pad.pointer.device) {
			case MOUSEAXIS: {
				pad.pointer.value[0] = app->cached_pointer[0];
				pad.pointer.value[1] = app->cached_pointer[1];
				pad.pointer.value[2] = UpdatGetInput(app, input_test, 15, CONTROLAXIS, pad.pointer.device_number,0);
				break;
			}
			default:
				break;
	...
	for (size_t i = 0; i < 6; i++) {	
		if ( GetInputImage( buffer, pad.name, pad.left_stick.device, pad.left_stick.device_number, pad.left_stick.sym[i] ) ) {
			...
			if ( ptr ) {
				...
				pad.left_stick.sprite[i] = {0};
			}
		}
	}
}

uint8_t CodeOne_FallbackLoop( AppOne * app, GameOne * game, uint8_t state) {
	
	DrawTextSimple(app, &app->font_bitmap, "Code not loaded", { 40,40, 100, 100}, 1.0f, 0);
	return state;
};
...
void UpdateGameCodeFunction(CodeOne & platform) {
	...
	if ( !platform.Frame ) { platform.Frame = &CodeOne_FallbackLoop; }
...
#if PLATFORM_LINUX
	static char PLATFORM_CODE_FILENAME[] = "game-debug-linux-x86_64.so";
#else
	static char PLATFORM_CODE_FILENAME[] = "game-debug-windows-x86_64.dll";
#endif

void LoadGameCode(CodeOne & platform) {
	char handle_fullpath[512] = {0};
	SDL_snprintf(platform.handle_filename, 32, PLATFORM_CODE_FILENAME);
	
	
	#ifdef _DEBUG
	char workinghandle_fullpath[512] = {0};
	SDL_snprintf(workinghandle_fullpath, 512, "%s%s",platform.handle_filepath, platform.handle_filename);
	if (stat(workinghandle_fullpath, &platform.code_handle_time) < 0) {
		SDL_Log("GameCode time not found: %s", PLATFORM_CODE_FILENAME );
	}
	SDL_snprintf(handle_fullpath, 512, "%sgamecode_%x.tso",platform.handle_filepath, platform.counter++);
	//printf("handle_fullpath '%s'\n", handle_fullpath);
	//printf("workinghandle_fullpath '%s'\n", workinghandle_fullpath);
	remove(handle_fullpath);
	rename(workinghandle_fullpath, handle_fullpath);
	SDL_Delay(100);
	#else
	SDL_snprintf(handle_fullpath, 512, "%s%s",platform.handle_filepath, platform.handle_filename);
	#endif

	
	platform.handle = SDL_LoadObject(handle_fullpath);
	if ( !platform.handle  ) {
		printf("Loading '%s' failed: %s", handle_fullpath, SDL_GetError());
	}
...
void ReloadGameCode(CodeOne & platform, AppOne & app, GameOne & game) {
	char handle_fullpath[512] = {0};
	char workinghandle_fullpath[512] = {0};
	SDL_snprintf(workinghandle_fullpath, 512, "%s%s",platform.handle_filepath, platform.handle_filename);

	if (stat(workinghandle_fullpath, &platform.code_handle_check) == 0) {
...
void CleanupGameCode(CodeOne & platform) {
	...
	char workinghandle_fullpath[512] = {0};
	SDL_snprintf(workinghandle_fullpath, 512, "%s%s",platform.handle_filepath, platform.handle_filename);
	SDL_snprintf(handle_fullpath, 512, "%sgamecode_%x.tso",platform.handle_filepath, platform.counter - 1);

	//printf("handle_fullpath '%s'\n", handle_fullpath);
	//printf("workinghandle_fullpath '%s'\n", workinghandle_fullpath);

	rename(handle_fullpath, workinghandle_fullpath);
...
int main( int argc, char **argv ) {
	...
	//
	game.memory_size = 268435456;
	game.memory_pool = (uint8_t*)calloc(sizeof(uint8_t), game.memory_size);
	...
	if ( argc > 0 && argv[0] != nullptr	) {
		...
		while ( argv[0][counter] ) {
			if ( argv[0][counter] == ELIX_DIR_SEPARATOR ) {
				last_index = counter+1;
			}
			counter++;
		}
		if ( last_index != 0 && last_index < 512 ) {
			memcpy(platform.handle_filepath, argv[0], last_index);
		} else {
			memcpy(platform.handle_filepath, "./", 3);
		}
	}

	
	//TextureSet * tste1 = new(buf) TextureSet;
	//TextureSet * tste2 = new(buf) TextureSet;
	...
	app.window_dimension = {SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 720, 480};
	app.window = SDL_CreateWindow("Do You Want To Make An Millionaire", app.window_dimension.x, app.window_dimension.y, app.window_dimension.w, app.window_dimension.h, SDL_WINDOW_SHOWN|SDL_WINDOW_RESIZABLE);
	...
	app.texture_sheet = SDL_CreateTexture(app.renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STATIC, 128, 128);
	...
	SDL_SetTextureBlendMode( app.texture_sheet, SDL_BLENDMODE_NONE);
	if ( SDL_UpdateTexture(app.texture_sheet, nullptr, &playsheet_texture, 128*4) ) {
		SDL_Log("SDL_UpdateTexture: %s", SDL_GetError());
	}
	...
	for (size_t i = 1; i < 5; i++) {
		app.control_pads[i].button[0].device_number = i - 1;
		app.control_pads[i].button[1].device_number = i - 1;
		app.control_pads[i].button[2].device_number = i - 1;
		app.control_pads[i].button[3].device_number = i - 1;
		app.control_pads[i].button[4].device_number = i - 1;
		app.control_pads[i].button[5].device_number = i - 1;
		app.control_pads[i].button[6].device_number = i - 1;
		app.control_pads[i].button[7].device_number = i - 1;
		app.control_pads[i].button[8].device_number = i - 1;
		app.control_pads[i].button[9].device_number = i - 1;
		app.control_pads[i].left_stick.device_number = i - 1;
		app.control_pads[i].right_stick.device_number = i - 1;
	}
	...
	while(state) {
		...
		SDL_RenderClear(app.renderer);
		#ifdef _DEBUG
		ReloadGameCode(platform,app, game);
		#endif
		...
		//DrawTextSimple(&app, &app.font_bitmap, debug, {0, app.viewpoint.h - 10, 40, 20}, 1.0f, 0);
